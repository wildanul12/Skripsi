...
<?php $meteran = isset($_GET['meteran']) ? $_GET['meteran'] : null; ?>
<?php $sql = mysqli_query($conn, "SELECT sensor.meteran, monitoring.nama, monitoring.alamat, monitoring.golongan, sensor.liter 
		FROM sensor INNER JOIN monitoring ON sensor.meteran = monitoring.meteran WHERE sensor.meteran = '$meteran' ORDER BY id_sensor DESC LIMIT 1"); ?>
<!DOCTYPE html>
<html>
<head>
...
<form action="update_monitoring.php" method="GET">
...
$no=1;
$query = mysqli_query($conn, "SELECT SUM(liter) AS total FROM `sensor` WHERE meteran = '$meteran' ORDER BY id_sensor DESC LIMIT 1");
...
<?php include 'koneksi.php'; ?>
<?php $golongan = isset($_GET['golongan']) ? $_GET['golongan'] : null; ?>
<?php $sql = mysqli_query($conn, "SELECT * FROM sensor WHERE meteran = '$meteran'"); ?>
...
<?php while($output = mysqli_fetch_array($sql)){ ?>
	<tr>
		<td align="center"><?php echo $no++;?></td>
		<td align="center"><?php echo $output['meteran'];?></td>
		<td align="center"><?php echo $output['debit'];?></td>
		<td align="center"><?php echo $output['volume'];?></td>
		<td align="center"><?php echo $output['liter'];?></td>
	</tr>
<?php } ?>
<?php
					$query = mysqli_query($conn, "SELECT SUM(liter) AS total FROM `sensor` WHERE meteran = '$meteran'");
					$fetch = mysqli_fetch_array($query); {
?>
<tr>
